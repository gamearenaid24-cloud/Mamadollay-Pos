  ...
  return (
    <div className="space-y-6 max-w-[1400px] mx-auto">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        {cards.map((card, i) => (
          <motion.div
            ...
            key={card.title}
            className={cn(
              "bg-white p-4 md:p-5 rounded-xl border border-slate-200 shadow-sm",
              card.alert && "border-l-4 border-l-red-500"
            )}
          >
            <div className="text-slate-500 text-[9px] md:text-[10px] font-bold uppercase tracking-wider mb-1">{card.title}</div>
            <div className={cn("text-xl md:text-2xl font-bold tracking-tight", card.alert ? "text-red-600" : "text-slate-900")}>
              {card.value}
            </div>
            <div className={cn(
              "text-[9px] md:text-[10px] font-bold mt-2",
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
        <div className="lg:col-span-2 bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col min-h-[300px] md:min-h-[400px]">
          <div className="p-4 md:p-5 border-b border-slate-100 flex justify-between items-center">
            <h3 className="font-bold text-slate-800 text-xs md:text-sm uppercase tracking-tight">Recent Sales Activity</h3>
            <button className="text-blue-600 text-[10px] md:text-xs font-bold hover:underline">View All</button>
          </div>
            ...
            <div className="inline-block min-w-full align-middle">
              <table className="min-w-full text-left border-collapse">
                <thead className="bg-slate-50 text-slate-500 text-[10px] font-bold uppercase border-b border-slate-100">
                  <tr>
                    <th className="px-4 md:px-6 py-3 whitespace-nowrap">Invoice</th>
                    <th className="px-4 md:px-6 py-3 text-center whitespace-nowrap">Outlet</th>
                    <th className="px-4 md:px-6 py-3 text-right whitespace-nowrap">Amount</th>
                    <th className="px-4 md:px-6 py-3 text-center whitespace-nowrap">Status</th>
                  </tr>
                </thead>
                <tbody className="text-xs text-slate-700 divide-y divide-slate-50">
                  {recentSales.map((row, i) => (
                    <tr key={i} className="hover:bg-slate-50 transition-colors">
                      <td className="px-4 md:px-6 py-4 font-mono font-bold text-slate-900 whitespace-nowrap">{row.inv}</td>
                      <td className="px-4 md:px-6 py-4 text-center whitespace-nowrap">{row.outlet}</td>
                      <td className="px-4 md:px-6 py-4 text-right font-bold text-slate-900 whitespace-nowrap">{formatCurrency(row.amount)}</td>
                      <td className="px-4 md:px-6 py-4 text-center">
                        <span className={cn(
                          "px-2 py-1 rounded-full text-[9px] font-bold whitespace-nowrap",
